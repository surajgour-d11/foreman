---
name: setup
description: Check and fix the prerequisites for the foreman team plugin - required plugins, macOS tools, gh auth, Agent Teams, resilience and notification settings, and leftovers from a manual install. Run once after installing foreman, and again whenever something seems off.
---
...
3. Otherwise, for each problem the doctor can fix, ask the manager with AskUserQuestion (multiSelect) which to apply:
   ...
   - Resilience and notification settings (`--resilience`)
