...
| Trust boundary | A repo's ledger text is untrusted input. Both hooks that read it fence it as data, strip control and markup characters — `pre-compact.sh` allows only printable ASCII — and cap what they print before it reaches the lead: `session-start.sh` fences at most five ledgers with phases cut to 120 characters, `pre-compact.sh` one ledger. The manager's unfenced startup message carries a count and no repo text at all, since prose needs no structural character to forge authority. |
